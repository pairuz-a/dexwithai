...
local function main()
	...
	Explorer.UpdateView = function()
		local uiScale = 1.175
		local maxNodes = math.ceil(treeFrame.AbsoluteSize.Y / (20 * uiScale))
		local maxX = treeFrame.AbsoluteSize.X / uiScale
	...
	Explorer.NewListEntry = function(index)
		...
		newEntry.InputBegan:Connect(function(input)
			...
			service.TweenService:Create(newEntry.Indent,TweenInfo.new(0.15,Enum.EasingStyle.Quart,Enum.EasingDirection.Out),{BackgroundColor3 = Settings.Theme.Button, BackgroundTransparency = 0}):Play()
		end)
		...
		newEntry.InputEnded:Connect(function(input)
			...
			service.TweenService:Create(newEntry.Indent,TweenInfo.new(0.18,Enum.EasingStyle.Quart,Enum.EasingDirection.Out),{BackgroundTransparency = 1}):Play()
		end)
	...
	Explorer.Refresh = function()
		local uiScale = 1.175
		local maxNodes = math.max(math.ceil((treeFrame.AbsoluteSize.Y) / (20 * uiScale)), 0)	
		...
		for i = 1,maxNodes do
			...
			if node then
				...
				entry.Indent.Position = UDim2.new(0,depth,0,1)
				entry.Indent.Size = UDim2.new(1,-depth-2,1,-2)
	...
	Explorer.InitRightClick = function()
		...
		context:Register("TELEPORT_TO",{Name = "Teleport To", IconMap = Explorer.MiscIcons, Icon = "TeleportTo", OnClick = function()
			...
			for _,node in next, sList do
				...
				if targetPos then
					plrRP.CFrame = CFrame.new(targetPos + offset)
					break
				end
			end
		end})
...
return search]==]
	...
	Explorer.InitEntryTemplate = function()
		entryTemplate = create({
			...
			{2,"Frame",{BackgroundColor3=Color3.new(0.04313725605607,0.35294118523598,0.68627452850342),BackgroundTransparency=1,BorderColor3=Color3.new(0.33725491166115,0.49019610881805,0.73725491762161),BorderSizePixel=0,Name="Indent",Parent={1},Position=UDim2.new(0,20,0,1),Size=UDim2.new(1,-22,1,-2),}},
			{7,"UICorner",{CornerRadius=UDim.new(0,5),Parent={2},}},
			...
			{4,"TextButton",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,ClipsDescendants=true,Font=3,Name="Expand",Parent={2},Position=UDim2.new(0,-20,0,0),Size=UDim2.new(0,20,0,18),Text="",TextSize=14,}},
			{5,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Image="rbxassetid://5642383285",ImageRectOffset=Vector2.new(144,16),ImageRectSize=Vector2.new(16,16),Name="Icon",Parent={4},Position=UDim2.new(0,2,0,1),ScaleType=4,Size=UDim2.new(0,16,0,16),}},
			{6,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,ImageRectOffset=Vector2.new(304,0),ImageRectSize=Vector2.new(16,16),Name="Icon",Parent={2},Position=UDim2.new(0,4,0,1),ScaleType=4,Size=UDim2.new(0,16,0,16),}},
		})
	...
	Explorer.Init = function()
		...
		local explorerItems = create({
			...
			{2,"Frame",{BackgroundColor3=Color3.fromRGB(26,26,32),BorderSizePixel=0,Name="ToolBar",Parent={1},Size=UDim2.new(1,0,0,26),}},
			{3,"Frame",{BackgroundColor3=Color3.fromRGB(38,38,48),BorderColor3=Color3.fromRGB(20,20,26),BorderSizePixel=0,Name="SearchFrame",Parent={2},Position=UDim2.new(0,6,0,4),Size=UDim2.new(1,-12,0,18),}},
			{4,"TextBox",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,ClearTextOnFocus=false,Font=Enum.Font.Gotham,Name="SearchBox",Parent={3},PlaceholderColor3=Color3.fromRGB(130,130,140),PlaceholderText="Search workspace",Position=UDim2.new(0,8,0,0),Size=UDim2.new(1,-28,0,18),Text="",TextColor3=Color3.fromRGB(236,236,240),TextSize=12,TextXAlignment=0,}},
			{5,"UICorner",{CornerRadius=UDim.new(0,6),Parent={3},}},
			{6,"UIStroke",{Thickness=1,Parent={3},Color=Color3.fromRGB(54,54,66),Transparency=0.5}},
		...
		local explorerScale = Instance.new("UIScale")
		explorerScale.Scale = 1.175
		explorerScale.Parent = window.GuiElems.Content
...
local function main()
	...
	Lib.Window = (function()
		...
		local leftSide = {Width = 380, Windows = {}, ResizeCons = {}, Hidden = true}
		local rightSide = {Width = 380, Windows = {}, ResizeCons = {}, Hidden = true}

		local displayOrderStart
		local sideDisplayOrder
		local sideTweenInfo = TweenInfo.new(0.32,Enum.EasingStyle.Quart,Enum.EasingDirection.Out)
		local tweens = {}
		local isA = game.IsA

		local theme = {
			MainColor1 = Color3.fromRGB(30,30,36),
			MainColor2 = Color3.fromRGB(22,22,28),
		...
		local createGui = function(self)
			local gui = create({
				...
				{100,"Frame",{BackgroundColor3=Color3.fromRGB(22,22,28),BorderSizePixel=0,ClipsDescendants=true,Name="Wrapper",Parent={2},Size=UDim2.new(1,0,1,0),}},
				{9,"UICorner",{CornerRadius=UDim.new(0,8),Parent={100},}},
				{3,"Frame",{BackgroundColor3=Color3.fromRGB(22,22,28),BorderSizePixel=0,Name="Content",Parent={100},Position=UDim2.new(0,0,0,20),Size=UDim2.new(1,0,1,-20),ClipsDescendants=true}},
				{4,"Frame",{BackgroundColor3=Color3.fromRGB(46,46,56),BorderSizePixel=0,Name="Line",Parent={3},Size=UDim2.new(1,0,0,1),}},
				{5,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.fromRGB(30,30,36),BorderSizePixel=0,Name="TopBar",Parent={100},Size=UDim2.new(1,0,0,20),Text = ""}},
				{6,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=Enum.Font.GothamMedium,Name="Title",Parent={5},Position=UDim2.new(0,8,0,0),Size=UDim2.new(1,-16,0,20),Text="Window",TextColor3=Color3.fromRGB(236,236,240),TextSize=12,TextXAlignment=0}},
				{7,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.fromRGB(120,60,66),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="Close",Parent={5},Position=UDim2.new(1,-18,0,2),Size=UDim2.new(0,16,0,16),Text="",TextColor3=Color3.new(1,1,1),TextSize=14,}},
				...
				{101,"UICorner",{CornerRadius=UDim.new(1,0),Parent={7},}},
				{10,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.fromRGB(70,70,80),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="Minimize",Parent={5},Position=UDim2.new(1,-36,0,2),Size=UDim2.new(0,16,0,16),Text="",TextColor3=Color3.new(1,1,1),TextSize=14,}},
				...
				{12,"UICorner",{CornerRadius=UDim.new(1,0),Parent={10},}},
				{13,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,BorderSizePixel=0,Image="rbxassetid://1427967925",Name="Outlines",Parent={2},Position=UDim2.new(0,-5,0,-5),ScaleType=1,Size=UDim2.new(1,10,1,10),SliceCenter=Rect.new(6,6,25,25),TileSize=UDim2.new(0,20,0,20),ImageTransparency=1,}},
			...
			local guiTopBar = guiMain.Wrapper.TopBar
			local guiResizeControls = guiMain.ResizeControls

			self.GuiElems.Main = guiMain
			self.GuiElems.Wrapper = guiMain.Wrapper
			self.GuiElems.TopBar = guiMain.Wrapper.TopBar
			self.GuiElems.Content = guiMain.Wrapper.Content
			self.GuiElems.Line = guiMain.Wrapper.Content.Line
			...
			self.ContentPane = guiMain.Wrapper.Content
			...
			self.GuiElems.Content.BackgroundTransparency = 0
		...
		static.Init = function()
			...
			local leftFrame = create({
				{1,"Frame",{Active=true,Name="LeftSide",BackgroundColor3=Color3.fromRGB(22,22,28),BackgroundTransparency=1,BorderSizePixel=0,}},
				{2,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.fromRGB(52,52,62),BorderSizePixel=0,Font=3,Name="Resizer",Parent={1},Size=UDim2.new(0,5,1,0),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,BackgroundTransparency=1,}},
				{3,"Frame",{BackgroundColor3=Color3.fromRGB(38,38,46),BorderSizePixel=0,Name="Line",Parent={2},Position=UDim2.new(0,0,0,0),Size=UDim2.new(0,1,1,0),BackgroundTransparency=1,}},
				{4,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.fromRGB(52,52,62),BorderSizePixel=0,Font=3,Name="WindowResizer",Parent={1},Position=UDim2.new(1,-300,0,0),Size=UDim2.new(1,0,0,4),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,BackgroundTransparency=0,}},
				{5,"Frame",{BackgroundColor3=Color3.fromRGB(38,38,46),BorderSizePixel=0,Name="Line",Parent={4},Size=UDim2.new(1,0,0,1),BackgroundTransparency=0,}},
			})
			...
			local rightFrame = create({
				{1,"Frame",{Active=true,Name="RightSide",BackgroundColor3=Color3.fromRGB(22,22,28),BackgroundTransparency=1,BorderSizePixel=0,}},
				{2,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.fromRGB(52,52,62),BorderSizePixel=0,Font=3,Name="Resizer",Parent={1},Size=UDim2.new(0,5,1,0),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,BackgroundTransparency=1,}},
				{3,"Frame",{BackgroundColor3=Color3.fromRGB(38,38,46),BorderSizePixel=0,Name="Line",Parent={2},Position=UDim2.new(0,4,0,0),Size=UDim2.new(0,1,1,0),BackgroundTransparency=1,}},
				{4,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.fromRGB(52,52,62),BorderSizePixel=0,Font=3,Name="WindowResizer",Parent={1},Position=UDim2.new(1,-300,0,0),Size=UDim2.new(1,0,0,4),Text="",TextColor3=Color3.new(0,0,0),TextSize=14,BackgroundTransparency=0,}},
				{5,"Frame",{BackgroundColor3=Color3.fromRGB(38,38,46),BorderSizePixel=0,Name="Line",Parent={4},Size=UDim2.new(1,0,0,1),BackgroundTransparency=0,}},
			})
			...
			-- transparency override intentionally disabled; side panels stay opaque
			...
			local leftToggle = create({
				{1,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.fromRGB(30,30,36),BorderColor3=Color3.fromRGB(20,20,26),BorderMode=2,Font=Enum.Font.GothamBold,Name="LeftToggle",Position=UDim2.new(0,0,0,-36),Size=UDim2.new(0,16,0,36),Text="<",TextColor3=Color3.fromRGB(236,236,240),TextSize=13,}},
			...
			Lib.ButtonAnim(leftToggle,{Mode = 2,PressColor = Color3.fromRGB(42,42,52)})
			Lib.ButtonAnim(rightToggle,{Mode = 2,PressColor = Color3.fromRGB(42,42,52)})
	...
	Lib.CodeFrame = (function()
		...
		local function makeFrame(obj)
			...
			-- transparency override intentionally disabled; frame stays opaque
	...
	Lib.Checkbox = (function()
		...
		local function initGui(self,frame)
			local checkbox = frame or create({
				...
				{3,"Frame",{BackgroundColor3=Color3.fromRGB(46,46,58),BorderSizePixel=0,Name="outline",Parent={1},Size=UDim2.new(0,16,0,16),}},
				{4,"Frame",{BackgroundColor3=Color3.fromRGB(24,24,30),BorderSizePixel=0,Name="filler",Parent={3},Position=UDim2.new(0,1,0,1),Size=UDim2.new(0,14,0,14),}},
				...
				{13,"UICorner",{CornerRadius=UDim.new(0,5),Parent={3},}},
				{14,"UICorner",{CornerRadius=UDim.new(0,4),Parent={4},}},
			})
		...
		funcs.Collapse = function(self,anim)
			...
			if anim then
				TweenSize(guiElems.Top, ud2o(14, 14), "In", "Quart", 2/15, true)
				TweenSize(guiElems.Bottom, ud2o(14, 14), "In", "Quart", 2/15, true)
				TweenSize(guiElems.Left, ud2o(14, 14), "In", "Quart", 2/15, true)
				TweenSize(guiElems.Right, ud2o(14, 14), "In", "Quart", 2/15, true)
			else
		...
		funcs.Expand = function(self,anim)
			local guiElems = self.GuiElems
			if anim then
				TweenSize(guiElems.Top, ud2o(14, 0), "InOut", "Quart", 2/15, true)
				TweenSize(guiElems.Bottom, ud2o(14, 0), "InOut", "Quart", 2/15, true)
				TweenSize(guiElems.Left, ud2o(0, 14), "InOut", "Quart", 2/15, true)
				TweenSize(guiElems.Right, ud2o(0, 14), "InOut", "Quart", 2/15, true)
			else
		...
		funcs.Paint = function(self)
			...
			if self.Style == 0 then
				...
				if self.Disabled then
					guiElems.Outline.BackgroundColor3 = self.Colors.Disabled
					guiElems.Filler.BackgroundColor3 = self.Colors.DisabledBackground
					guiElems.Checkmark2.ImageColor3 = self.Colors.DisabledCheck or self.Colors.Primary
				elseif self.Toggled then
					guiElems.Outline.BackgroundColor3 = self.Colors.Primary
					guiElems.Filler.BackgroundColor3 = self.Colors.Primary
					guiElems.Checkmark2.ImageColor3 = Color3.new(1,1,1)
				else
					guiElems.Outline.BackgroundColor3 = self.Colors.Secondary
					guiElems.Filler.BackgroundColor3 = self.Colors.Background
					guiElems.Checkmark2.ImageColor3 = self.Colors.Primary
				end
			end
		end
		...
		funcs.SetState = function(self,val,anim)
			...
			if self.Toggled then
				if self.Style == 0 then
					if anim then
						self.OutlineColorTween = service.TweenService:Create(self.GuiElems.Outline, ti(2/15, Enum.EasingStyle.Circular, Enum.EasingDirection.Out), {BackgroundColor3 = self.Colors.Primary})
				...
				if self.Style == 0 then
					if anim then
						self.OutlineColorTween = service.TweenService:Create(self.GuiElems.Outline, ti(2/15, Enum.EasingStyle.Circular, Enum.EasingDirection.In), {BackgroundColor3 = self.Colors.Secondary})
						self.OutlineColorTween:Play()
						delay(0.08, function()
							if setStateTime ~= self.LastSetStateTime then return end
							self:Paint()
		...
		local function new(style)
			local obj = setmetatable({
				...
				Style = style or 0,
				Colors = {
					Background = c3(24,24,30),
					Primary = c3(49,176,230),
					Secondary = c3(68,68,80),
...
local function main()
	...
	Properties.UpdateView = function()
		local uiScale = 1.175
		local maxEntries = math.floor(propsFrame.AbsoluteSize.Y / (23 * uiScale))
		local maxX = propsFrame.AbsoluteSize.X / uiScale
		local totalWidth = Properties.ViewWidth + Properties.MinInputWidth

		scrollV.VisibleSpace = maxEntries
		scrollV.TotalSpace = #viewList + 2
		scrollH.VisibleSpace = maxX
		scrollH.TotalSpace = totalWidth

		scrollV.Gui.Visible = #viewList + 2 > maxEntries
		scrollH.Gui.Visible = totalWidth > maxX
	...
	Properties.Refresh = function()
		local uiScale = 1.175
		local maxEntries = math.max(math.floor((propsFrame.AbsoluteSize.Y) / (23 * uiScale)),0)	
	...
	Properties.InitEntryStuff = function()
		...
		-- transparency override intentionally disabled; property rows stay opaque
	...
	Properties.Init = function() -- TODO: MAKE BETTER
		local guiItems = create({
			...
			{2,"Frame",{BackgroundColor3=Color3.fromRGB(26,26,32),BorderSizePixel=0,Name="ToolBar",Parent={1},Size=UDim2.new(1,0,0,26),}},
			{3,"Frame",{BackgroundColor3=Color3.fromRGB(38,38,48),BorderColor3=Color3.fromRGB(20,20,26),BorderSizePixel=0,Name="SearchFrame",Parent={2},Position=UDim2.new(0,6,0,4),Size=UDim2.new(1,-12,0,18),}},
			{4,"TextBox",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,ClearTextOnFocus=false,Font=Enum.Font.Gotham,Name="SearchBox",Parent={3},PlaceholderColor3=Color3.fromRGB(130,130,140),PlaceholderText="Search properties",Position=UDim2.new(0,8,0,0),Size=UDim2.new(1,-28,0,18),Text="",TextColor3=Color3.fromRGB(236,236,240),TextSize=12,TextXAlignment=0,}},
			{5,"UICorner",{CornerRadius=UDim.new(0,6),Parent={3},}},
			{12,"UIStroke",{Thickness=1,Parent={3},Color=Color3.fromRGB(54,54,66),Transparency=0.5}},
			...
			{11,"Frame",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,ClipsDescendants=true,Name="List",Parent={1},Position=UDim2.new(0,0,0,27),Size=UDim2.new(1,0,1,-27),}},
		})
		...
		local propsScale = Instance.new("UIScale")
		propsScale.Scale = 1.175
		propsScale.Parent = window.GuiElems.Content
...
DefaultSettings = (function()
	...
	return {
		...
		AI = {
			_Recurse = true,
			ApiKey = "",
			ApiKeys = "", -- multiple keys, one per line
			Model = "gemini-3.6-flash",
			ThinkingLevel = "high",
			MaxOutputTokens = 32768,
			MaxIterations = math.huge,
			AllowExecuteScript = false, -- security: AI can run code
			AllowFireRemote = false, -- security: AI can fire remotes
			AllowSetProperty = false, -- security: AI can set properties
			AllowModifyInstances = false, -- security: AI can clone/destroy
			AllowTeleport = false, -- security: AI can teleport player
		},
		ClassIcon = "NewDark",
...
Main = (function()
	...
	Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
		local gui = create({
			...
			{7,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=Enum.Font.GothamMedium,Name="StatusText",Parent={3},Position=UDim2.new(0,0,0,110),Size=UDim2.new(1,0,0,20),Text="Fetching API",TextColor3=Color3.fromRGB(200,200,210),TextSize=13,TextTransparency=1,TextXAlignment=2,}},
			{8,"Frame",{BackgroundColor3=Color3.fromRGB(46,46,58),BorderSizePixel=0,Name="ProgressBar",Parent={3},Position=UDim2.new(0.5,-50,0,148),Size=UDim2.new(0,0,0,4),}},
			...
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=Enum.Font.Gotham,Name="Creator",Parent={2},Position=UDim2.new(1,-130,1,-24),Size=UDim2.new(0,120,0,18),Text="Developed by Papapai?.",TextColor3=Color3.fromRGB(180,180,190),TextSize=12,TextXAlignment=1,}},
		...
		tweenS:Create(statusText,progressTI,{Position = UDim2.new(0,0,0,124), TextTransparency = 0}):Play()
		tweenS:Create(progressBar,progressTI,{Position = UDim2.new(0.5,-50,0,148), Size = UDim2.new(0,100,0,4)}):Play()
	...
	Main.AI = (function()
		...
		local Models = {
			...
			{Id = "deepseek-3.2", Label = "Deepseek 3.2 (Free)"},
			...
			{Id = "glm-5", Label = "GLM-5 (Free)"},
			{Id = "kimi-k2.7-code", Label = "Kimi K2.7 Code"},
			{Id = "mimo-v2.5", Label = "Mimo V2.5"},
			{Id = "mimo-v2.5-pro", Label = "Mimo V2.5 Pro"},
			{Id = "minimax-m2.5", Label = "MiniMax M2.5 (Free)"},
			{Id = "nemotron-3-super", Label = "Nemotron 3 Super (Free)"},
		...
		tools.get_script_source = {
			...
			run = function(args)
				...
				if #src > 6000 then src = src:sub(1, 6000) .. "\n-- [truncated at 6k chars, use open_script_in_notepad for full source]" end
		...
		tools.search = {
			description = "Search the entire DataModel for instances whose Name matches the query (case-insensitive substring). Returns up to `limit` results. IMPORTANT: Call this tool ONLY ONCE per user request with the broadest possible query. Do NOT call search multiple times in a row for related terms — if you need to find several things, use a single short common substring or call list_children instead. Prefer limit=60 unless the user needs fewer.",
			parameters = {
				type = "object",
				properties = {
					query = {type = "string"},
		...
		tools.execute_script = {
			description = "Execute a Lua script in the game environment using loadstring. Returns the output (print captures) or error. Use this to manipulate instances, fire remotes, change properties, teleport, etc. Be careful with destructive operations. REQUIRES AllowExecuteScript setting to be enabled.",
			parameters = {
				type = "object",
				properties = {
			...
			run = function(args)
				if not Settings.AI.AllowExecuteScript then
					return {error = "Execute Script is disabled. Enable it in AI Settings."}
				end
				if not args.code or args.code == "" then return {error = "no code provided"} end
				local output = {}
				...
				if not fn then
					return {error = "compile error: " .. tostring(compileErr)}
				end
				setfenv(fn, fakeEnv)
				local ok, runtimeErr = pcall(fn)
				if not ok then
					return {error = "runtime error: " .. tostring(runtimeErr), output = output}
				end
		...
		tools.fire_remote = {
			description = "Fire a RemoteEvent or invoke a RemoteFunction at the given path with optional arguments. REQUIRES AllowFireRemote setting to be enabled.",
			parameters = {
				type = "object",
				properties = {
					path = {type = "string", description = "Instance path to the RemoteEvent or RemoteFunction."},
					args = {type = "array", items = {type = "string"}, description = "Arguments to pass (strings, numbers, booleans)."}
				},
				required = {"path"}
			},
			run = function(args)
				if not Settings.AI.AllowFireRemote then
					return {error = "Fire Remote is disabled. Enable it in AI Settings."}
				end
				local inst, err = resolvePath(args.path)
				if err then return {error = err} end
				if inst:IsA("RemoteEvent") then
					local ok, fireErr = pcall(function() inst:FireServer(unpack(args.args or {})) end)
					if not ok then return {error = tostring(fireErr)} end
					return {fired = true, path = getPath(inst), type = "RemoteEvent"}
				elseif inst:IsA("RemoteFunction") then
					local ok, result = pcall(function() return inst:InvokeServer(unpack(args.args or {})) end)
					if not ok then return {error = tostring(result)} end
					return {invoked = true, path = getPath(inst), type = "RemoteFunction", result = tostring(result)}
				end
				return {error = "instance is not a RemoteEvent or RemoteFunction"}
			end
		}

		tools.read_console = {
			description = "Read the latest output from the in-game console/output log. Returns recent messages (print, warn, error).",
			parameters = {
				type = "object",
				properties = {
					limit = {type = "integer", description = "Maximum number of recent messages to return (default 30)."}
				},
				required = {}
			},
			run = function(args)
				...
				local messages = {}
				if Console and Console.Logs then
					local startIdx = math.max(1, #Console.Logs - limit + 1)
					for i = startIdx, #Console.Logs do
						local log = Console.Logs[i]
						if log then
							table.insert(messages, {type = log.Type or "info", message = tostring(log.Message or log.Text or log)})
						end
					end
				elseif service.LogService then
					local ok, logs = pcall(function() return service.LogService:GetLogHistory() end)
					if ok and logs then
						local startIdx = math.max(1, #logs - limit + 1)
						for i = startIdx, #logs do
							local entry = logs[i]
							table.insert(messages, {type = tostring(entry.messageType), message = entry.message, timestamp = entry.timestamp})
						end
					end
				end
				return {count = #messages, messages = messages}
			end
		}

		tools.set_property = {
			description = "Set a property on an instance. REQUIRES AllowSetProperty setting to be enabled.",
			parameters = {
				type = "object",
				properties = {
					path = {type = "string", description = "Instance path."},
					property = {type = "string", description = "Property name to set."},
					value = {type = "string", description = "Value to set (string, number, boolean). For Vector3 use format 'x,y,z', for Color3 use 'r,g,b' with 0-1 range."}
				},
				required = {"path", "property", "value"}
			},
			run = function(args)
				if not Settings.AI.AllowSetProperty then
					return {error = "Set Property is disabled. Enable it in AI Settings."}
				end
				local inst, err = resolvePath(args.path)
				if err then return {error = err} end
				...
				if type(val) == "string" then
					...
					elseif val:match("^[%d%.%-]+,[%d%.%-]+,[%d%.%-]+$") then
						local parts = string.split(val, ",")
						val = Vector3.new(tonumber(parts[1]) or 0, tonumber(parts[2]) or 0, tonumber(parts[3]) or 0)
					end
				elseif type(val) == "table" then
					if val.x and val.y and val.z then
						val = Vector3.new(val.x, val.y, val.z)
					elseif val.r and val.g and val.b then
						val = Color3.new(val.r, val.g, val.b)
					end
				end
				local ok, setErr = pcall(function() inst[args.property] = val end)
				if not ok then return {error = tostring(setErr)} end
				return {success = true, path = getPath(inst), property = args.property, new_value = tostring(val)}
			end
		}

		tools.clone_instance = {
			description = "Clone an instance and parent it to a target. REQUIRES AllowModifyInstances setting to be enabled.",
			parameters = {
				type = "object",
				properties = {
					path = {type = "string", description = "Instance path to clone."},
					parent_path = {type = "string", description = "Path to parent the clone into (optional, defaults to same parent)."}
				},
				required = {"path"}
			},
			run = function(args)
				if not Settings.AI.AllowModifyInstances then
					return {error = "Modify Instances is disabled. Enable it in AI Settings."}
				end
				local inst, err = resolvePath(args.path)
				if err then return {error = err} end
				local parent = inst.Parent
				if args.parent_path then
					local p, pErr = resolvePath(args.parent_path)
					if pErr then return {error = "parent: " .. pErr} end
					parent = p
				end
				...
				if not ok then return {error = tostring(clone)} end
				return {success = true, cloned = getPath(clone), parent = getPath(parent)}
			end
		}

		tools.destroy_instance = {
			description = "Destroy an instance permanently. REQUIRES AllowModifyInstances setting to be enabled.",
			parameters = {
				type = "object",
				properties = {
			...
			run = function(args)
				...
				local ok, dErr = pcall(function() inst:Destroy() end)
				if not ok then return {error = tostring(dErr)} end
				return {success = true, destroyed = path}
			end
		}

		tools.teleport_player = {
			description = "Teleport the local player's character to a position (Vector3) or to another instance's position. REQUIRES AllowTeleport setting to be enabled.",
			parameters = {
				type = "object",
				properties = {
					position = {type = "string", description = "Target position as 'x,y,z' format (e.g. '100,50,200')."},
					target_path = {type = "string", description = "Instance path to teleport to (uses its Position or CFrame)."}
				},
				required = {}
			},
			run = function(args)
				if not Settings.AI.AllowTeleport then
					return {error = "Teleport is disabled. Enable it in AI Settings."}
				end
				if not plr.Character then return {error = "no character"} end
				local hrp = plr.Character:FindFirstChild("HumanoidRootPart")
				if not hrp then return {error = "no HumanoidRootPart"} end
				local targetCF
				if args.target_path then
					local inst, err = resolvePath(args.target_path)
					if err then return {error = err} end
					if inst:IsA("BasePart") then
						targetCF = inst.CFrame
					elseif inst:IsA("Model") then
						local ok, pivot = pcall(function() return inst:GetPivot() end)
						if ok then targetCF = pivot else return {error = "cannot get model pivot"} end
					else
						return {error = "target is not a BasePart or Model"}
					end
				elseif args.position then
					...
					targetCF = CFrame.new(x, y, z)
				else
					return {error = "provide position or target_path"}
				end
				local ok, tpErr = pcall(function() hrp.CFrame = targetCF end)
				if not ok then return {error = tostring(tpErr)} end
				return {success = true, teleported_to = tostring(targetCF.Position)}
			end

			tools.write_script = {
				description = "Create a new LocalScript or Script in the game with the provided Luau code. Use this when the user asks you to write a script — always create it instead of just replying with code. REQUIRES AllowModifyInstances setting to be enabled.",
    parameters = {
        type = "object",
        properties = {
            name = {type = "string", description = "Name for the new script (e.g. 'AutoRoadwork')."},
            code = {type = "string", description = "The full Luau code to put in the script."},
            script_type = {type = "string", description = "'LocalScript' or 'Script'. Default is 'LocalScript'."},
            parent_path = {type = "string", description = "Where to parent the script. Defaults to 'game.Players.LocalPlayer.PlayerScripts'."}
        },
        required = {"name", "code"}
    },
    run = function(args)
        ...
        local scriptType = args.script_type or "LocalScript"
        if scriptType ~= "LocalScript" and scriptType ~= "Script" then
            return {error = "script_type must be 'LocalScript' or 'Script'"}
        end
        local parent
        if args.parent_path then
            local inst, err = resolvePath(args.parent_path)
            if err then return {error = "parent: " .. err} end
            parent = inst
        else
            parent = plr and plr:FindFirstChild("PlayerScripts") or game:GetService("Players").LocalPlayer.PlayerScripts
        end
        ...
        if not ok then return {error = tostring(result)} end
        return {success = true, path = getPath(result), name = result.Name, type = scriptType}
    end
			}
...
- NEVER call search more than once per turn. Use a single broad query and filter the results yourself. If you need to find multiple unrelated things, pick the most relevant one or use list_children.
...
- Your responses are rendered with markdown: **bold**, *italic*, `inline code`, and fenced ```code blocks``` all work. Use them to make output readable but don't over-format.]]
		...
		local function callGemini(model, contents, apiKey)
			...
			local body = {
				model = model,
				messages = messages,
				max_tokens = Settings.AI.MaxOutputTokens or 32768,
				temperature = 0.4,
			...
			local ok, resp = pcall(env.request, {
				Url = "https://api.cosmoshub.tech/v1/chat/completions",
				Method = "POST",
				Headers = {
			...
			if resp.StatusCode and resp.StatusCode >= 400 then
				return nil, "HTTP " .. tostring(resp.StatusCode) .. ": " .. tostring(resp.Body):sub(1,400), resp.StatusCode
			end
			...
			if decoded.error then return nil, "API error: " .. tostring(decoded.error.message or "unknown"), tonumber(decoded.error.code) end
			return decoded
		end
		...
		local function addBubble(kind, text)
			...
			elseif kind == "tool" then
				...
				-- Action buttons (Copy / Regenerate) under bot messages
				local actionsFrame = Instance.new("Frame", bubble)
				actionsFrame.BackgroundTransparency = 1
				actionsFrame.Size = UDim2.new(1,-16,0,24)
				actionsFrame.Position = UDim2.new(0,8,1,-28)
				actionsFrame.AutomaticSize = Enum.AutomaticSize.None
				actionsFrame.LayoutOrder = 999
				...
				-- Copy button
				local copyBtn = Instance.new("TextButton", actionsFrame)
				copyBtn.BackgroundTransparency = 1
				copyBtn.Font = Enum.Font.Gotham
				copyBtn.TextSize = 11
				copyBtn.TextColor3 = ThemeSubtle
				copyBtn.Size = UDim2.new(0,44,0,20)
				copyBtn.Text = "Copy"
				copyBtn.AutoButtonColor = false
				copyBtn.MouseButton1Click:Connect(function()
					if env.setclipboard then
						pcall(env.setclipboard, text or "")
						copyBtn.Text = "Copied!"
						task.delay(1.5, function() copyBtn.Text = "Copy" end)
					end
				end)

				-- Adjust label padding to make room for actions
				bLabelPad.PaddingBottom = UDim.new(0,30)
			end
		...
		local function getApiKeys()
			local keys = {}
			local multiRaw = Settings.AI.ApiKeys or ""
			for line in multiRaw:gmatch("[^\n]+") do
				local k = line:match("^%s*(.-)%s*$")
				if k and k ~= "" then table.insert(keys, k) end
			end
			local singleKey = Settings.AI.ApiKey or ""
			if singleKey ~= "" then
				local found = false
				for _, k in ipairs(keys) do if k == singleKey then found = true break end end
				if not found then table.insert(keys, 1, singleKey) end
			end
			return keys
		end

		local function addToolBubble(toolName, argsText)
			if not AI.ChatMessages then return end
			local frame = Instance.new("Frame")
			frame.BackgroundTransparency = 1
			frame.Size = UDim2.new(1,-16,0,0)
			...
			frame.Size = UDim2.new(1,-16,0,headerHeight)
			...
			table.insert(AI.MessageBubbles, {frame = frame, kind = "tool"})
			task.spawn(function()
				task.wait()
				if AI.ChatMessages then
					AI.ChatMessages.CanvasPosition = Vector2.new(0, AI.ChatMessages.AbsoluteCanvasSize.Y)
		...
		function AI.RunAgentLoop(userMessage)
			...
			local apiKeys = getApiKeys()
			if #apiKeys == 0 then
				addBubble("bot", "⚠ Please set your CosmosHub API key in the Settings panel (⚙ button).")
				AI.Running = false
				setStatus("")
				AI.UpdateSendButton()
				return
			...
			local model = Settings.AI.Model or "gemini-3.5-flash"

			table.insert(AI.History, {role = "user", content = userMessage})

			local iterations = 0
			local maxIter = math.huge -- unlimited iterations
			while iterations < maxIter and not AI.SessionCancelled do
				iterations = iterations + 1

				-- Try each key, rotate on quota errors
				local decoded, err, statusCode
				local allFailed = true
				for _, key in ipairs(apiKeys) do
					if AI.SessionCancelled then break end
					decoded, err, statusCode = callGemini(model, AI.History, key)
					if decoded then
						allFailed = false
						break
					elseif statusCode == 429 and #apiKeys > 1 then
						-- quota error, try next key silently
						decoded = nil
					else
						allFailed = true
						break
				...
				if not decoded then
					-- Only show error if all keys failed or single key
					if allFailed then
				...
				if not choice or not choice.message then
					addBubble("bot", "⚠ Empty response from AI.")
					break
				end
				...
				if message.content and message.content ~= "" and message.content ~= nil then
					table.insert(textOut, message.content)
				end
				...
				if message.tool_calls then
					for _, tc in ipairs(message.tool_calls) do
						...
						if tc["function"] and tc["function"].arguments then
							local ok, parsed = pcall(function() return httpService:JSONDecode(tc["function"].arguments) end)
				...
				table.insert(AI.History, {role = "assistant", content = message.content or "", tool_calls = message.tool_calls})
				...
				for _, fc in ipairs(functionCalls) do
					...
					addToolBubble(fc.name, httpService:JSONEncode(fc.args or {}):sub(1,200))
		...
		function AI.UpdateSendButton()
			if not AI.SendButton then return end
			if AI.Running then
				AI.SendButton.Text = "Stop"
				AI.SendButton.BackgroundColor3 = Color3.fromRGB(220,50,50)
			else
				AI.SendButton.Text = "Send"
				AI.SendButton.BackgroundColor3 = ThemeAccent
			end
		end
		...
		function AI.Init()
			...
			-- Settings bar (settings gear button + clear + status)
			local settingsBar = Instance.new("Frame")
			settingsBar.BackgroundColor3 = Color3.fromRGB(26,26,32)
			settingsBar.BorderSizePixel = 0
			settingsBar.Size = UDim2.new(1,0,0,36)
			settingsBar.Parent = content

			-- Settings gear button
			local settingsBtn = Instance.new("TextButton", settingsBar)
			settingsBtn.BackgroundColor3 = ThemeInputBg
			settingsBtn.BorderSizePixel = 0
			settingsBtn.Font = Enum.Font.GothamBold
			settingsBtn.TextSize = 14
			settingsBtn.TextColor3 = ThemeText
			settingsBtn.Position = UDim2.new(0,10,0,6)
			settingsBtn.Size = UDim2.new(0,28,0,24)
			settingsBtn.AutoButtonColor = false
			settingsBtn.Text = "⚙"
			local sgCorner = Instance.new("UICorner", settingsBtn)
			sgCorner.CornerRadius = UDim.new(0,6)

			-- Key count badge
			local keyCountLabel = Instance.new("TextLabel", settingsBar)
			keyCountLabel.BackgroundTransparency = 1
			keyCountLabel.Font = Enum.Font.GothamMedium
			keyCountLabel.TextSize = 11
			keyCountLabel.TextColor3 = ThemeSubtle
			keyCountLabel.TextXAlignment = Enum.TextXAlignment.Left
			keyCountLabel.Position = UDim2.new(0,44,0,12)
			keyCountLabel.Size = UDim2.new(0,80,0,12)
			local function refreshKeyCount()
				local keys = getApiKeys()
				if #keys > 0 then
					keyCountLabel.Text = #keys .. " key" .. (#keys > 1 and "s" or "")
				else
					keyCountLabel.Text = "No keys"
				end
			end
			refreshKeyCount()
			...
			clearBtn.Position = UDim2.new(0,130,0,10)
			clearBtn.Size = UDim2.new(0,70,0,16)
			...
			statusLabel.Position = UDim2.new(0.5,4,0,10)
			statusLabel.Size = UDim2.new(0.5,-14,0,16)
			statusLabel.Text = ""
			AI.StatusLabel = statusLabel

			-- ===== SETTINGS POPUP =====
			local settingsPopup = Instance.new("Frame")
			settingsPopup.BackgroundColor3 = Color3.fromRGB(20,20,26)
			settingsPopup.BorderSizePixel = 0
			settingsPopup.Size = UDim2.new(1,-20,0,480)
			settingsPopup.Position = UDim2.new(0,10,0,40)
			settingsPopup.Visible = false
			settingsPopup.ZIndex = 10
			settingsPopup.Parent = content
			local spCorner = Instance.new("UICorner", settingsPopup)
			spCorner.CornerRadius = UDim.new(0,10)
			local spStroke = Instance.new("UIStroke", settingsPopup)
			spStroke.Color = Color3.fromRGB(60,60,70)
			spStroke.Thickness = 1

			-- Title
			local popTitle = Instance.new("TextLabel", settingsPopup)
			popTitle.BackgroundTransparency = 1
			popTitle.Font = Enum.Font.GothamBold
			popTitle.TextSize = 14
			popTitle.TextColor3 = ThemeText
			popTitle.Position = UDim2.new(0,14,0,10)
			popTitle.Size = UDim2.new(1,-28,0,20)
			popTitle.TextXAlignment = Enum.TextXAlignment.Left
			popTitle.Text = "AI Settings"
			popTitle.ZIndex = 11

			-- Model selection
			local modelLbl = Instance.new("TextLabel", settingsPopup)
			modelLbl.BackgroundTransparency = 1
			modelLbl.Font = Enum.Font.GothamMedium
			modelLbl.TextSize = 11
			modelLbl.TextColor3 = ThemeSubtle
			modelLbl.TextXAlignment = Enum.TextXAlignment.Left
			modelLbl.Position = UDim2.new(0,14,0,36)
			modelLbl.Size = UDim2.new(1,-28,0,14)
			modelLbl.Text = "MODEL"
			modelLbl.ZIndex = 11

			local modelButton = Instance.new("TextButton", settingsPopup)
			...
			modelButton.Position = UDim2.new(0,14,0,52)
			modelButton.Size = UDim2.new(1,-28,0,24)
			modelButton.AutoButtonColor = false
			modelButton.ZIndex = 11
			...
			-- API Keys label
			local keysLbl = Instance.new("TextLabel", settingsPopup)
			keysLbl.BackgroundTransparency = 1
			keysLbl.Font = Enum.Font.GothamMedium
			keysLbl.TextSize = 11
			keysLbl.TextColor3 = ThemeSubtle
			keysLbl.TextXAlignment = Enum.TextXAlignment.Left
			keysLbl.Position = UDim2.new(0,14,0,84)
			keysLbl.Size = UDim2.new(1,-28,0,14)
			keysLbl.Text = "API KEYS (one per line)"
			keysLbl.ZIndex = 11

			local keysBox = Instance.new("TextBox", settingsPopup)
			keysBox.BackgroundColor3 = ThemeInputBg
			keysBox.BorderSizePixel = 0
			keysBox.Font = Enum.Font.Code
			keysBox.TextSize = 11
			keysBox.TextColor3 = ThemeText
			keysBox.PlaceholderText = "Paste API keys here, one per line..."
			keysBox.PlaceholderColor3 = Color3.fromRGB(100,100,110)
			keysBox.TextXAlignment = Enum.TextXAlignment.Left
			keysBox.TextYAlignment = Enum.TextYAlignment.Top
			keysBox.ClearTextOnFocus = false
			keysBox.MultiLine = true
			keysBox.TextWrapped = true
			keysBox.Position = UDim2.new(0,14,0,100)
			keysBox.Size = UDim2.new(1,-28,0,140)
			keysBox.ZIndex = 11
			local kbCorner = Instance.new("UICorner", keysBox)
			kbCorner.CornerRadius = UDim.new(0,6)
			local kbPad = Instance.new("UIPadding", keysBox)
			...
			local existingKeys = Settings.AI.ApiKeys or ""
			if existingKeys == "" and Settings.AI.ApiKey and Settings.AI.ApiKey ~= "" then
				existingKeys = Settings.AI.ApiKey
			end
			keysBox.Text = existingKeys

			-- Security toggles
			local toggleY = 248
			local toggles = {
				{key = "AllowExecuteScript", label = "Allow AI Execute Script"},
				{key = "AllowFireRemote", label = "Allow AI Fire Remote"},
				{key = "AllowSetProperty", label = "Allow AI Set Property"},
				{key = "AllowModifyInstances", label = "Allow AI Clone/Destroy"},
				{key = "AllowTeleport", label = "Allow AI Teleport"},
			}
			local toggleButtons = {}
			for idx, tog in ipairs(toggles) do
				local yPos = toggleY + (idx-1) * 26

				local togBtn = Instance.new("TextButton", settingsPopup)
				togBtn.BackgroundColor3 = Settings.AI[tog.key] and Color3.fromRGB(10,132,255) or Color3.fromRGB(60,60,70)
				togBtn.BorderSizePixel = 0
				togBtn.Font = Enum.Font.GothamBold
				togBtn.TextSize = 10
				togBtn.TextColor3 = Color3.new(1,1,1)
				togBtn.Size = UDim2.new(0,16,0,16)
				togBtn.Position = UDim2.new(0,14,0,yPos)
				togBtn.Text = Settings.AI[tog.key] and "ON" or ""
				togBtn.AutoButtonColor = false
				togBtn.ZIndex = 11
				local tCorner = Instance.new("UICorner", togBtn)
				tCorner.CornerRadius = UDim.new(0,4)

				local togLabel = Instance.new("TextLabel", settingsPopup)
				togLabel.BackgroundTransparency = 1
				togLabel.Font = Enum.Font.Gotham
				togLabel.TextSize = 12
				togLabel.TextColor3 = ThemeText
				togLabel.TextXAlignment = Enum.TextXAlignment.Left
				togLabel.Position = UDim2.new(0,36,0,yPos)
				togLabel.Size = UDim2.new(1,-50,0,16)
				togLabel.Text = tog.label
				togLabel.ZIndex = 11

				togBtn.MouseButton1Click:Connect(function()
					Settings.AI[tog.key] = not Settings.AI[tog.key]
					togBtn.BackgroundColor3 = Settings.AI[tog.key] and Color3.fromRGB(10,132,255) or Color3.fromRGB(60,60,70)
					togBtn.Text = Settings.AI[tog.key] and "ON" or ""
				end)

				toggleButtons[tog.key] = togBtn
			end

			-- Save button
			local saveBtn = Instance.new("TextButton", settingsPopup)
			saveBtn.BackgroundColor3 = ThemeAccent
			...
			saveBtn.Position = UDim2.new(0,14,0,396)
			saveBtn.Size = UDim2.new(1,-28,0,30)
			saveBtn.ZIndex = 11
			local svCorner = Instance.new("UICorner", saveBtn)
			svCorner.CornerRadius = UDim.new(0,8)

			saveBtn.MouseButton1Click:Connect(function()
				local rawText = keysBox.Text or ""
				Settings.AI.ApiKeys = rawText
				-- Also set first key as the legacy single key for compatibility
				local firstKey = ""
				for line in rawText:gmatch("[^\n]+") do
					local k = line:match("^%s*(.-)%s*$")
					if k and k ~= "" then firstKey = k break end
				end
				Settings.AI.ApiKey = firstKey
			...
			local closePopBtn = Instance.new("TextButton", settingsPopup)
			closePopBtn.BackgroundTransparency = 1
			closePopBtn.Font = Enum.Font.GothamBold
			closePopBtn.TextSize = 16
			closePopBtn.TextColor3 = ThemeSubtle
			closePopBtn.Position = UDim2.new(1,-32,0,6)
			closePopBtn.Size = UDim2.new(0,24,0,24)
			closePopBtn.Text = "X"
			closePopBtn.ZIndex = 11
			closePopBtn.MouseButton1Click:Connect(function()
				settingsPopup.Visible = false
			end)
			...
			settingsBtn.MouseButton1Click:Connect(function()
				...
				if settingsPopup.Visible then
					local existKeys = Settings.AI.ApiKeys or ""
					if existKeys == "" and Settings.AI.ApiKey and Settings.AI.ApiKey ~= "" then
						existKeys = Settings.AI.ApiKey
					end
					keysBox.Text = existKeys
					refreshModelLabel()
				end
			end)
			...
			chatFrame.Position = UDim2.new(0,0,0,38)
			chatFrame.Size = UDim2.new(1,0,1,-(38 + 48))
			...
			-- Input area
			local INPUT_MIN_H = 32
			local INPUT_MAX_H = 300
			local INPUT_BAR_PAD = 16

			local inputBar = Instance.new("Frame")
			inputBar.BackgroundColor3 = Color3.fromRGB(26,26,32)
			inputBar.BorderSizePixel = 0
			inputBar.Position = UDim2.new(0,0,1,-(INPUT_MIN_H + INPUT_BAR_PAD))
			inputBar.Size = UDim2.new(1,0,0,INPUT_MIN_H + INPUT_BAR_PAD)
			...
			inputBox.PlaceholderText = "js type w y wan brh"
			...
			inputBox.Size = UDim2.new(1,-90,0,INPUT_MIN_H)
			...
			local ibSizeConstraint = Instance.new("UISizeConstraint", inputBox)
			ibSizeConstraint.MinSize = Vector2.new(0, INPUT_MIN_H)
			ibSizeConstraint.MaxSize = Vector2.new(math.huge, INPUT_MAX_H)

			AI.InputBox = inputBox

			local function updateInputBarSize()
				task.wait()
				local boxH = math.clamp(inputBox.AbsoluteSize.Y, INPUT_MIN_H, INPUT_MAX_H)
				local barH = boxH + INPUT_BAR_PAD
				inputBar.Size = UDim2.new(1,0,0,barH)
				inputBar.Position = UDim2.new(0,0,1,-barH)
				chatFrame.Size = UDim2.new(1,0,1,-(38 + barH))
				if AI.SendButton then
					AI.SendButton.Position = UDim2.new(1,-74,0, math.floor((barH - 28)/2))
				end
			end

			inputBox:GetPropertyChangedSignal("AbsoluteSize"):Connect(updateInputBarSize)
			...
			-- Shift+Enter = yeni satir, Enter = gonder
			service.UserInputService.InputBegan:Connect(function(input, gameProcessed)
				if not inputBox:IsFocused() then return end
				if input.KeyCode == Enum.KeyCode.Return and not service.UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) and not service.UserInputService:IsKeyDown(Enum.KeyCode.RightShift) then
					task.defer(function()
			...
			addBubble("bot", "we script sum gang 😭😭")
		end
	...
	Main.ScriptExec = (function()
		local Executor = {}

		function Executor.Init()
			local window = Lib.Window.new()
			window:SetTitle("Script Executor")
			...
			codeBox.BackgroundColor3 = Color3.fromRGB(32,32,40)
			codeBox.BorderSizePixel = 0
			codeBox.Font = Enum.Font.Code
			codeBox.TextSize = 13
			codeBox.TextColor3 = Color3.fromRGB(220,220,230)
			codeBox.PlaceholderText = "-- Write Lua code here..."
			...
			codeBox.Size = UDim2.new(1,-16,1,-52)
			codeBox.Text = ""
			...
			cbPad.PaddingRight = UDim.new(0,10)
			cbPad.PaddingTop = UDim.new(0,8)
			cbPad.PaddingBottom = UDim.new(0,8)
			Executor.CodeBox = codeBox

			-- Execute button
			local execBtn = Instance.new("TextButton", content)
			execBtn.BackgroundColor3 = Color3.fromRGB(10,132,255)
			...
			execBtn.Position = UDim2.new(0,8,1,-38)
			execBtn.Size = UDim2.new(1,-86,0,30)
			...
			execBtn.MouseButton1Click:Connect(function()
				...
				if not fn then
					warn("[Executor] Compile error: " .. tostring(err))
					return
				end
				local ok, runtimeErr = pcall(fn)
				if not ok then
					warn("[Executor] Runtime error: " .. tostring(runtimeErr))
				end
			end)
			...
			clearBtn.Text = "Clear"
			clearBtn.AutoButtonColor = false
			clearBtn.Position = UDim2.new(1,-68,1,-38)
	...
	Main.CreateMainGui = function()
		...
		Main.ScriptExec.Init()
		Main.CreateApp({Name = "Executor", IconMap = Main.LargeIcons, Icon = "Output", Window = Main.ScriptExec.Window})
