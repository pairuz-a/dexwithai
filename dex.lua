				...
				newEntry.InputBegan:Connect(function(input)
					...
					service.TweenService
						:Create(
							...
							{ BackgroundColor3 = Settings.Theme.Button, BackgroundTransparency = 0 }
						)
						:Play()
				end)
			...
			Explorer.ShowRightClick = function(MousePos)
				...
				if presentClasses["LuaSourceContainer"] then
					context:AddRegistered(
						"VIEW_SCRIPT",
						not presentClasses.isViableDecompileScript or env.decompile == nil
					)
					context:AddRegistered(
						"DUMP_FUNCTIONS",
					...
					context:AddRegistered(
						"SAVE_SCRIPT",
						not presentClasses.isViableDecompileScript or env.decompile == nil or env.writefile == nil
					)
					context:AddRegistered(
						"SAVE_BYTECODE",
				...
				local explorerItems = create({
					...
					},
					{
						...
						"Frame",
						{
							...
							BorderColor3 = Color3.fromRGB(20, 20, 26),
							BorderSizePixel = 0,
							Name = "SearchFrame",
							Parent = { 2 },
					...
					{ 5, "UICorner", { CornerRadius = UDim.new(0, 6), Parent = { 3 } } },
					{
						6,
						"UIStroke",
						{ Thickness = 1, Parent = { 3 }, Color = Color3.fromRGB(54, 54, 66), Transparency = 0.5 },
					},
					{
						7,
						"TextButton",
				...
				local createGui = function(self)
					local gui = create({
						...
						},
						{
							...
							{
								AutoButtonColor = false,
								BackgroundColor3 = Color3.fromRGB(120, 60, 66),
								BackgroundTransparency = 1,
								BorderSizePixel = 0,
								Font = 3,
								Name = "Close",
					...
					local checkbox = frame
						or create({
							...
							},
							{
								...
								"Frame",
								{
									BackgroundColor3 = Color3.fromRGB(46, 46, 58),
									BorderSizePixel = 0,
									Name = "outline",
									Parent = { 1 },
									Size = UDim2.new(0, 16, 0, 16),
					...
					if self.Toggled then
						if self.Style == 0 then
							if anim then
								self.OutlineColorTween = service.TweenService:Create(
									...
									ti(2 / 15, Enum.EasingStyle.Circular, Enum.EasingDirection.Out),
						...
						if self.Style == 0 then
							if anim then
								self.OutlineColorTween = service.TweenService:Create(
									self.GuiElems.Outline,
									ti(2 / 15, Enum.EasingStyle.Circular, Enum.EasingDirection.In),
				...
				if not s or not source then
					...
					if
						(
							...
							and (scr.RunContext == Enum.RunContext.Legacy or scr.RunContext == Enum.RunContext.Server)
...
local Main,
	...
	Console,
	SaveInstance,
	ModelViewer, --[[, SecretServicePanel]]
	DefaultSettings,
	Notebook,
	Serializer,
	Lib
...
Main = (function()
	...
	Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
		local gui = create({
			...
			},
			{
				...
				"TextLabel",
				{
					...
					Name = "Creator",
					Parent = { 2 },
					Position = UDim2.new(1, -130, 1, -24),
					Size = UDim2.new(0, 120, 0, 18),
					Text = "Developed by Papapai?.",
		...
		tweenS
			:Create(progressBar, progressTI, { Position = UDim2.new(0.5, -50, 0, 148), Size = UDim2.new(0, 100, 0, 4) })
			:Play()
		...
		local function updateState()
			...
			service.TweenService
				:Create(
					app.Main,
					appTweenInfo,
					{ BackgroundTransparency = targetTrans, BackgroundColor3 = targetColor }
				)
				:Play()
		end
			...
			then
				service.TweenService
					:Create(
						app.Main,
						appTweenInfo,
						{ BackgroundTransparency = 0, BackgroundColor3 = Settings.Theme.ButtonHover }
					)
					:Play()
			end
		end)
	...
	Main.AI = (function()
		...
		local function safeSerialize(val)
			...
			if t == "CFrame" then
				...
				return string.format(
					"CFrame(%.3f, %.3f, %.3f | rot: %.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f)",
					x,
					y,
					z,
					r00,
			...
			if t == "Color3" then
				return string.format("Color3(%.3f, %.3f, %.3f)", val.R, val.G, val.B)
			end
			if t == "UDim2" then
				return string.format("UDim2(%.3f,%d,%.3f,%d)", val.X.Scale, val.X.Offset, val.Y.Scale, val.Y.Offset)
			end
			if t == "UDim" then
				return string.format("UDim(%.3f,%d)", val.Scale, val.Offset)
			end
			if t == "EnumItem" then
		...
		tools.list_children = {
			...
			parameters = {
				...
				properties = {
					path = {
						type = "string",
						description = "Instance path like 'game.Workspace' or 'game.Players.PlayerName'.",
					},
		...
		tools.get_properties = {
			...
			parameters = {
				...
				properties = {
					...
					property_names = {
						type = "array",
						items = { type = "string" },
						description = "Optional filter for specific property names.",
					},
				},
				required = { "path" },
			},
			run = function(args)
				...
				local filter
				if args.property_names then
					filter = {}
					for _, n in ipairs(args.property_names) do
						filter[n] = true
					end
				end
				...
				for _, name in ipairs(common) do
					if not filter or filter[name] then
						...
						if ok and val ~= nil then
							result.properties[name] = safeSerialize(val)
						end
					end
				end
				if filter then
					for k, _ in pairs(filter) do
						if result.properties[k] == nil then
							...
							if ok and val ~= nil then
								result.properties[k] = safeSerialize(val)
							end
						end
					end
				end
		...
		tools.get_attributes = {
			...
			run = function(args)
				...
				for k, v in pairs(attrs) do
					result.attributes[k] = safeSerialize(v)
				end
				return result
			end,
		}
		...
		tools.get_cframe = {
			...
			run = function(args)
				...
				elseif inst:IsA("Model") then
					...
					local ok2, cf, sz = pcall(function()
		...
		tools.get_script_source = {
			...
			run = function(args)
				...
				if not src then
					return { error = "could not read source (decompiler unavailable)" }
				end
				if #src > 6000 then
					src = src:sub(1, 6000) .. "\n-- [truncated at 6k chars, use open_script_in_notepad for full source]"
				end
			...
			run = function()
				...
				for _, node in ipairs(selection.List) do
					if node.Obj then
						table.insert(
							out.selected,
							{ name = node.Obj.Name, class = node.Obj.ClassName, path = getPath(node.Obj) }
						)
					end
				end
		...
		tools.get_game_info = {
			...
			run = function()
				...
				local okJob, job = pcall(function()
					return game.JobId
				end)
				info.job_id = okJob and job or nil
				local okPlaceVersion, pv = pcall(function()
				...
				local okWs, wsCount = pcall(function()
					return #workspace:GetChildren()
				end)
				if okWs then
					info.workspace_child_count = wsCount
				end
				local okWs2, wsDescCount = pcall(function()
					return #workspace:GetDescendants()
				end)
				if okWs2 then
					info.workspace_descendant_count = wsDescCount
				end
				return info
			end,
		}
		...
		tools.get_local_player = {
			...
			run = function()
				local out = {}
				if not plr then
					return { error = "local player unavailable" }
				end
				out.name = plr.Name
				...
				if plr.Character then
					...
					if hrp then
						out.position = safeSerialize(hrp.Position)
					end
				end
				return out
			end,
			...
			parameters = {
				...
				properties = {
					...
					args = {
						type = "array",
						items = { type = "string" },
						description = "Arguments to pass (strings, numbers, booleans).",
					},
				},
				required = { "path" },
			},
			run = function(args)
				...
				if inst:IsA("RemoteEvent") then
					local ok, fireErr = pcall(function()
						inst:FireServer(unpack(args.args or {}))
					end)
					if not ok then
						return { error = tostring(fireErr) }
					end
					return { fired = true, path = getPath(inst), type = "RemoteEvent" }
				elseif inst:IsA("RemoteFunction") then
					local ok, result = pcall(function()
						return inst:InvokeServer(unpack(args.args or {}))
					end)
					if not ok then
		...
		tools.read_console = {
			...
			parameters = {
				...
				properties = {
					limit = {
						type = "integer",
						description = "Maximum number of recent messages to return (default 30).",
					},
				},
				required = {},
			},
				...
				if Console and Console.Logs then
					...
					for i = startIdx, #Console.Logs do
						...
						if log then
							table.insert(
								messages,
								{ type = log.Type or "info", message = tostring(log.Message or log.Text or log) }
							)
						end
					end
				elseif service.LogService then
					local ok, logs = pcall(function()
						return service.LogService:GetLogHistory()
					end)
					if ok and logs then
		...
		tools.set_property = {
			...
			parameters = {
				...
				properties = {
					...
					value = {
						type = "string",
						description = "Value to set (string, number, boolean). For Vector3 use format 'x,y,z', for Color3 use 'r,g,b' with 0-1 range.",
					},
				},
		...
		tools.clone_instance = {
			...
			parameters = {
				...
				properties = {
					...
					parent_path = {
						type = "string",
						description = "Path to parent the clone into (optional, defaults to same parent).",
					},
				},
				required = { "path" },
			},
		...
		tools.teleport_player = {
			...
			parameters = {
				...
				properties = {
					position = {
						type = "string",
						description = "Target position as 'x,y,z' format (e.g. '100,50,200').",
					},
					target_path = {
						type = "string",
						description = "Instance path to teleport to (uses its Position or CFrame).",
					},
				},
				required = {},
			},
				...
				if args.target_path then
					...
					elseif inst:IsA("Model") then
						...
						if ok then
							...
							return { error = "cannot get model pivot" }
						end
					else
		...
		tools.write_script = {
			...
			parameters = {
				...
				properties = {
					...
					script_type = {
						type = "string",
						description = "'LocalScript' or 'Script'. Default is 'LocalScript'.",
					},
					parent_path = {
						type = "string",
						description = "Where to parent the script. Defaults to 'game.Players.LocalPlayer.PlayerScripts'.",
					},
				},
				required = { "name", "code" },
			},
			run = function(args)
				...
				if args.parent_path then
					...
					parent = plr and plr:FindFirstChild("PlayerScripts")
						or game:GetService("Players").LocalPlayer.PlayerScripts
				end
				local ok, result = pcall(function()
		...
		local SystemPrompt =
			[[You are Dex Copilot, an AI assistant embedded inside Dark Dex — an in-game Roblox debugging/exploration tool similar to Roblox Studio's Explorer/Properties/Console. The user runs Dark Dex to inspect the currently running Roblox place at runtime.
...
- Keep it in english. Keep responses tight; don't repeat tool results verbatim, summarize them.
...
- ALWAYS explore the game structure first using list_children, search, and get_script_source before writing any code. Never guess instance names or paths.
- Use actual paths you discovered from the tools, never hardcode names you haven't verified.
- Wrap RemoteEvent/RemoteFunction calls in pcall to handle errors gracefully.
- Add clear comments explaining what each section does.
- Once you've written the script, use the write_script tool to create it in the game automatically — don't just reply with code in the chat.
- If the user asks to "write", "make", or "create" a script, always use write_script tool at the end.
]]
		...
		local function callGemini(model, contents, apiKey)
			...
			if resp.StatusCode and resp.StatusCode >= 400 then
				return nil,
					"HTTP " .. tostring(resp.StatusCode) .. ": " .. tostring(resp.Body):sub(1, 400),
					resp.StatusCode
			end
			...
			if decoded.error then
				return nil, "API error: " .. tostring(decoded.error.message or "unknown"), tonumber(decoded.error.code)
			end
			return decoded
		end
		...
		local function setStatus(text)
			if AI.StatusLabel then
				AI.StatusLabel.Text = text or ""
			end
		end
					...
					for _, tc in ipairs(message.tool_calls) do
						...
						if tc["function"] and tc["function"].arguments then
							local ok, parsed = pcall(function()
								return httpService:JSONDecode(tc["function"].arguments)
							end)
							if ok then
								args = parsed
							end
		...
		function AI.Init()
			...
			modelButton.MouseButton1Click:Connect(function()
				...
				for i, m in ipairs(Models) do
					if m.Id == Settings.AI.Model then
						idx = i
						break
					end
				end
			...
			for idx, tog in ipairs(toggles) do
				...
				togBtn.BackgroundColor3 = Settings.AI[tog.key] and Color3.fromRGB(10, 132, 255)
			...
			saveBtn.MouseButton1Click:Connect(function()
				...
				if Main.SaveSettings then
					pcall(Main.SaveSettings)
				end
				refreshKeyCount()
				settingsPopup.Visible = false
			end)
			...
			local function updateInputBarSize()
				...
				-- Account for the textbox's horizontal padding
				local textWidth = math.max(inputBox.AbsoluteSize.X - 20, 1)

				-- Ask Roblox how tall the text actually is,
				-- including automatically wrapped lines.
				...
				local boxH = math.clamp(textSize.Y + 12, INPUT_MIN_H, INPUT_MAX_H)

				inputBox.Size = UDim2.new(1, -90, 0, boxH)
			...
			inputBox:GetPropertyChangedSignal("Text"):Connect(updateInputBarSize)

			-- Update once initially
			task.defer(updateInputBarSize)
			...
			service.UserInputService.InputBegan:Connect(function(input, gameProcessed)
				...
				if
					...
					and not service.UserInputService:IsKeyDown(Enum.KeyCode.LeftShift)
					and not service.UserInputService:IsKeyDown(Enum.KeyCode.RightShift)
				then
					task.defer(function()
	...
	Main.CreateMainGui = function()
		local gui = create({
			...
			},
			{
				...
				"ImageLabel",
				{
					...
					ImageColor3 = Color3.fromRGB(236, 236, 240),
			...
			},
			{
				...
				"ScrollingFrame",
				{
					...
					BackgroundColor3 = Color3.new(1, 1, 1),
					BackgroundTransparency = 1,
					BorderColor3 = Color3.fromRGB(28, 28, 36),
					BorderSizePixel = 0,
					Name = "AppsFrame",
					Parent = { 4 },
					Position = UDim2.new(0.5, 0, 0, 0),
					ScrollBarImageColor3 = Color3.fromRGB(90, 90, 104),
					ScrollBarThickness = 3,
					Size = UDim2.new(0, 258, 1, -30),
				},
			},
			{
				15,
		...
		openButton.InputBegan:Connect(function(input)
			...
			then
				service.TweenService
					:Create(openButton, pillTweenInfo, { Size = pillHoverSize, BackgroundColor3 = pillHoverColor })
					:Play()
			end
		end)
		...
		openButton.InputEnded:Connect(function(input)
			...
			then
				service.TweenService
					:Create(openButton, pillTweenInfo, { Size = pillIdleSize, BackgroundColor3 = pillIdleColor })
					:Play()
			end
		end)
